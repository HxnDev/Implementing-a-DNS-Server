			...
			for(i=0;i<MAXFD;++i)
			{
				...
				if(FD_ISSET(fds[i],&fdset))
				{
					if(fds[i]==sockfd)
					{
						...
						// creating c1 data and sending to proxy
						if (c1_port == 0) 
						{
							c1_port = (int)ntohs(caddr.sin_port);
							c1_str = "S1\tC1\t" + to_string(c1_port);
							c1_str += "\n";
							c1_fd = c;
							clientports.append(c1_str);

							string temp = "01C1 connected to S1";
							temp = temp + "+" + c1_str;
							send(proxy_fd, temp.c_str(), temp.size(), 0);
						}

						// creating c2 data and sending to proxy
						else if (c2_port == 0) 
						{
							c2_port = (int)ntohs(caddr.sin_port);
							c2_str = "S1\tC2\t" + to_string(c2_port);
							c2_str += "\n";
							c2_fd = c;
							clientports.append(c2_str);

							string temp = "01C2 connected to S1";
							temp = temp + "+" + c2_str;
							send(proxy_fd, temp.c_str(), temp.size(), 0);
						}

						// send ports to proxy when two clients connected to server
						if (c1_port and c2_port) {
					...
					else   //Receive data recv when an existing client sends data
					{
						...
						if(res<=0)
						{
							if (fds[i] == c1_fd) { // client 1 has exited
								clientports.clear();
								c1_str.clear();
								clientports = "Server\tClient\tPort\n";
								clientports.append("S1\tC2\t" + to_string(c2_port));
								clientports += "\n";
								c1_port = 0;
							}

							else if (fds[i] == c2_fd) { // client 2 has exited
								clientports.clear();
								c2_str.clear();
								clientports = "Server\tClient\tPort\n";
								clientports.append("S1\tC1\t" + to_string(c1_port));
								clientports += "\n";
								c2_port = 0;
							}
						...
						else
						{
							...
							//	send(proxy_fd, "5", 1, 0); 										// asking proxy for updated routing table
								...
								recv(fds[i],buff,127,0);										//Server will recieve the message of client 
								int p = atoi(buff);
								memset(buff,0,sizeof(buff));
							...
							// recieving client
								...
								if (client_port == to_string(c1_port))
									recv_fd = c1_fd;

								else if (client_port == to_string(c2_port))
									recv_fd = c2_fd;
